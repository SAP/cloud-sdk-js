...
- [connectivity] The generic types of `JwtKeyMapping` is simplified so the second type argument `JwtKeysT` are always strings. (300cb37bb)
- [connectivity] The `Protocol` enum was replaced with a string literal union type. Now, instead of passing `Protocol.HTTPS` pass 'https'. (300cb37bb)
- [connectivity, http-client, openapi, odata-common, odata-v2, resilience] Remove the options `timeout` and `enableCircuitBreaker` from all BTP service interactions i.e. destination and token fetching. (300cb37bb)
- [connectivity, http-client, mail-client, openapi, odata-common, odata-v2, odata-v4, eslint-config, generator, test-util, util, openapi-generator, generator-common, temporal-de-serializers, resilience] Switch the compilerOptions.module to `Node16` instead of `CommonJS` for better interoperability with ES modules. See the [TypeScript handbook](https://www.typescriptlang.org/docs/handbook/esm-node.html) for technical details if needed. (300cb37bb)
- [connectivity] The enum `IsolationStrategy` was replaced with a string literal union type of the same name. Use 'tenant' and 'tenant-user' instead of `IsolationStrategy.Tenant` and `IsolationStrategy.Tenant_User`. (300cb37bb)
- [connectivity, http-client, odata-common] ] The `url` property in `Destination` is now optional as destinations of type `MAIL` do not have a URL. (300cb37bb)
- [http-client, openapi, odata-common] Remove `timeout()` method from the request builders and the `timeout` options from the `executeHttpRequest()` function.
  Visit the [documentation portal](https://sap.github.io/cloud-sdk/docs/js/guides/resilience) to see how to add a timeout using middlewares. (300cb37bb)
- [http-client] Removed overload of executeHttpRequest
  The overload, that accepted `HttpRequestConfigWithOrigin` as a parameter, is removed and replaced by the function `executeHttpRequestWithOrigin`. (300cb37bb)
- [odata-common, generator, util] By default, generation of OData clients fails if a service path cannot be determined. Either provide `servicePath` value in the `options-per-service.json` or set `skipValidation` to true, in which case, `/` will be used. (300cb37bb)
- [odata-common] The constructor of `ODataRequestConfig` was changed so that the third parameter cannot be a `string` anymore.
  ...
  The type of the parameter is now `Record<string, any>`, and if only want to set the `Content-Type` HTTP header you can do so by passing `{'content-type': 'some-value'}` to the constructor. (300cb37bb)
- [odata-common] The constructor of the entity API is private to avoid accidental usage. Use the service object to get a API instance. (300cb37bb)
- [odata-common, generator, openapi-generator] The `serviceMapping` option of the OData generator has been renamed to `optionsPerService`. The mapping file, `service-mapping.json` has also been renamed to `options-per-service.json`. By default, an options file will not be generated. (300cb37bb)
- [odata-common] Setting custom fields in `fromJson` through the `_customFields` property has been removed.
  Add custom properties to your JSON object instead. (300cb37bb)
- [generator] The options per service behavior is now the same as for the OpenApi generator.
  For details on how to migrate and what has change look at the migration guide. (300cb37bb)
- [generator] The deprecated `forceOverwrite` option of the generator is removed. Use the `overwrite` option instead.
  ...
  [Compatibility Note] The deprecated `generateNpmrc` option of the generator is removed. Use the `include` option to add a `.npmrc` to the generated code if needed. (300cb37bb)
- [generator] Add `verbose` generator option. By default, only error and warning logs will be displayed. If `verbose` is set to `true`, all logs will be displayed. (300cb37bb)
- [generator] The `generateJs` option has been replaced with the `transpile` option. Transpilation is not enabled by default. (300cb37bb)
- [generator] The command line argument `inputDir` of the OData generator is renamed to `input`.
  The new `input` options accepts now also file paths and glob patterns. (300cb37bb)
- [generator] The OData generator won't automatically rename identifiers to avoid name conflicts.
  ...
  Switch on the `skipValidation` flag if you want to generate despite name conflicts and are okay with changed identifier names to avoid conflicts. (300cb37bb)
- [generator, openapi-generator, generator-common] Removed the option `versionInPackageJson` from the OData and OpenAPI generator.
  ...
  If necessary use the `include` option to add a `package.json` with a custom value. (300cb37bb)
- [generator, openapi-generator] Rename servicePath to basePath. (300cb37bb)
- [generator] The option `generateCSN` is removed. There is no replacement. (300cb37bb)
- [generator] The type for paths in the `GeneratorOptions` is changed from `fs.PathLike` to `string`.
  In case you passed a buffer object please resolve it to a string before passing it to the SAP Cloud SDK. (300cb37bb)
- [generator, openapi-generator, generator-common] The two generators use the same CLI parsing code now, aligning the way paths are resolved.
  In case you experience problems with the new logic, enable the `verbose` flag to investigate the new paths. (300cb37bb)
- [util] The function `variadicArgumentToArray` is replaced by the function `transformVariadicArgumentToArray`. (300cb37bb)
- [util] The field `logger` on the interface `LoggerOptions` was not used and is removed from the interface. (300cb37bb)
- [openapi-generator] The internal option `packageVersion` of the OpenAPI generator is removed. (300cb37bb)
- [openapi-generator] `tsConfig` option has been renamed to `tsconfig`. (300cb37bb)
- [generator-common] The generator does not create `d.ts.map` files per default anymore. If you need them include a custom `tsconfig.json`.
  ...
  Modern IDEs don't require those files for code navigation, thus they are removed to decrease download size of the SDK. (300cb37bb)

## New Functionalities

- [connectivity] Support `OAuth2RefreshToken` authentication type (300cb37bb)
- [connectivity] Add a `retry` option in the `DestinationFetchOption`.
  Enable this options if the token exchange done by the destination service is unstable. (300cb37bb)
- [connectivity, resilience] Add `ResilienceOptions` and `resilience()` function. The function returns an array of middlewares based on the provided options. (300cb37bb)
- [http-client, resilience] The request configuration used in the final request is now part of the middleware context.
  ...
  The request configuration contains the `url`, `headers` and all other properties of the HTTP request. (300cb37bb)
- [http-client, openapi, odata-common] Introduce the middleware on the request builders and http-client.
  Visit the [documentation portal](https://sap.github.io/cloud-sdk/docs/js/guides/resilience) to see how to use middlewares. (300cb37bb)
- [generator, generator-common] Introduced options `transpile` and `tsconfig` to configure transpilation for generated OData clients. (300cb37bb)

## Improvements

- [connectivity] Replace `Protocol` enum with a string literal union type. (300cb37bb)
- [connectivity] Replace `IsolationStrategy` enum with union type. (300cb37bb)
- [generator] Align naming rules for operations and properties in OData clients by removing `_` prefix (300cb37bb)

## Fixed Issues

- [connectivity] Fix the combination of providing the `iss` together with `OnPremise` basic destinations. (300cb37bb)
- [odata-v2] Allow to update OData v2 entities to `null`. Fixes [3204](https://github.com/SAP/cloud-sdk-js/issues/3204). (300cb37bb)
- [generator] Allow OData service to contain an entity name 'Service'. (300cb37bb)
- [generator] Now links to the correct generator binary. (300cb37bb)
