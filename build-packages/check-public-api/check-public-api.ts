 ...
 * @param pathsToIgnoreOnCheck - Optional parameter that ignores the files in the provided path during check.
...
function compareApisAndLog(
  ...
  allExportedTypes.forEach(exportedType => {
    ...
    const isPathMatched = pathsToIgnoreOnCheck?.length
      ? pathsToIgnoreOnCheck.some(ignoredPath =>
 ...
 * @param strict - Ensure that all APIs not marked public or internal are exported through internal.ts.
 * @param pathsToIgnoreOnCheck - Optional parameter that ignores the files in the provided path during check.
 */
export async function checkApiOfPackage(
  pathToPackage: string,
  strict: boolean,
  pathsToIgnoreOnCheck?: string[]
): Promise<void> {
  try {
    ...
    const allExportedIndex = await parseIndexFile(indexFilePath, strict);

    const setsAreEqual = compareApisAndLog(
      allExportedIndex,
      allExportedTypes,
      pathsToIgnoreOnCheck
    );
    mock.restore();
    if (!setsAreEqual) {
      process.exit(1);
...
export async function parseIndexFile(
  ...
  const starFileExports = await Promise.all(
    starFiles.map(async relativeFilePath =>
      parseIndexFile(resolve(cwd, `${relativeFilePath}.ts`), strict)
    )
  );
