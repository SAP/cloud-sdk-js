...
:::note Dependency version is handled via Cloud SDK BOM
When your project uses the SDK Bill-of-Material (BOM) `sdk-modules-bom` in the `dependencyManagement` section of the POM file, then you can omit the `version` tag. Otherwise, mention the latest SAP Cloud SDK version in the `version´ tag.
:::
...
:::tip
If your project was generated from one of Cloud SDK archetypes these dependencies should already present in your `pom.xml`
:::
