...
final ModificationResponse<BusinessPartner> deleteResult = response.getModificationResult(deleteBusinessPartner);
```

Similarly, use the method `getReadResult` to access response for a specific retrieve request.

```java
final List<BusinessPartners> readAllResult = response.getReadResult(getBusinessPartners);
final BusinessPartner readByKeyResult = response.getReadResult(getBusinessPartnerByKey);
```
