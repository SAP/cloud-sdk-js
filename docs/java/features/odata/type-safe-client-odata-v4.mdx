...
Multiple single requests (like `createBusinessPartner`) can be wrapped into so-called change sets.
...
final BatchResponse result =
    service
        .batch()
        .addReadOperations(businessPartners, businessPartnerByKey)
        .addChangeset(createBusinessPartner1(), createBusinessPartner2(), deleteBusinessPartner())
        .execute(destination);
```
...
```java
final ModificationResponse<BusinessPartner> createResult = response.getModificationResult(createBusinessPartner1);
final ModificationResponse<BusinessPartner> deleteResult = response.getModificationResult(        .addChangeset(createBusinessPartner1(), createBusinessPartner2(), deleteBusinessPartner()));
```

Similarly, use the method `getReadResult` to access response for a specific retrieve request.

```java
final List<BusinessPartners> readAllResult = response.getReadResult(businessPartners);
final BusinessPartner readByKeyResult = response.getReadResult(businessPartnerByKey);
```
