...
Caching may improve your applications responsiveness to your consumers.
...
```java
ResilienceDecorator.executeSupplier(() -> doStuff(), resilienceConfig);
ResilienceDecorator.executeSupplier(() -> doStuff(), resilienceConfig);
```

The above code will invoke `doStuff()` only once sicne the result of the first invocation will be cached.
...
// cachedOperation() will be evaludated
TenantAccessor.executeWithTenant(tenantA, () -> cachedOperation());
// cached value will be taken
TenantAccessor.executeWithTenant(tenantA, () -> cachedOperation());

// cachedOperation() will be evaludated because the cached value is only accessible for tenantA
TenantAccessor.executeWithTenant(tenantB, () -> cachedOperation());
// cached value for tenantB will be taken
TenantAccessor.executeWithTenant(tenantB, () -> cachedOperation());
```

The isolation is configured via the isolation mode of the resilience configuration.
See the section under [Resilience](https://sap.github.io/cloud-sdk/docs/java/features/resilience/resilience#multi-tenancy)
