...
Caching may improve your applications performance and responsiveness to your consumers.
...
<<<<<<< Updated upstream
The above code will invoke `operation()` only once sicne the result of the first invocation will be cached.
=======
The above code will invoke `doStuff()` only once since the result of the first invocation will be cached.
>>>>>>> Stashed changes
...
See the section on isolation levels under [Resilience](https://sap.github.io/cloud-sdk/docs/java/features/resilience/resilience#multi-tenancy)

## Manipulating the State of Caches

:::note
A specific cache instance is not yet created when configuration is created.
...
final Cache<SomeCacheKey, SomeResult> cache = Caching.getCachingProvider()
                                                     .getCacheManager()
                                                     .getCache(resilienceId) //Returns the cache created using the resilienceId passed
                                                     .registerCacheEntryListener(SimpleCacheEntryListener); //Registers the event listeners
```
