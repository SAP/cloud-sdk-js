...
Refer to the [migration guide](./features/odata/use-typed-odata-v2-client-in-sap-cloud-sdk-for-java#switch-to-improved-odata-v2-type-safe-client) for more details on the new OData 2 client.
  - The methods `executeRequest` are no longer marked as Beta and ready for use in production code.
  - The new methods `matchAnyVersionIdentifier()` and `disableVersionIdentifier()` allow for better control over [ETag handling](./features/odata/use-typed-odata-v2-client-in-sap-cloud-sdk-for-java#handling-of-etags).
