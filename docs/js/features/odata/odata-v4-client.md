...
The typed OData client allows us to build type-safe OData requests for a given service.
Every OData client generated by the SAP Cloud SDK provides request builders and abstractions for entity sets, that we refer to as entities. To create a request towards entity sets, entities offer a fluent API.
...
It is also possible to update an entity together with related entities in a single request, the same as [deep create](#deep-create).
-->
...
to add multiple request in the same batch function, I don't know what to add or how to add it
-->
