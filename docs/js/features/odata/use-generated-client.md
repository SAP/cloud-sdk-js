...
//TEMPORARY
    .requestType() // can be one of `getAll`, `getByKey`, etc. complete the list ;)
    .additionalRequestConfiguration() // describe it: custom headers, query parameters, e-tag handling, etc. details will follow (not sure how much you should already explain here, but it might make sense to give a few examples.
    .execute(myDestination);
```

- _requestType_ to construct requests for operations.
