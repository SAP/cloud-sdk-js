...
function updateTypeDocConfig() {
  transformFile('typedoc.json', config => JSON.stringify({ ...JSON.parse(config), out: `${path.relative(__dirname, apiDocPath)}/${version}` }, null, 2));
}
...
function generateDocs() {
  updateTypeDocConfig();
  const generationLogs = execSync('npx typedoc .', { encoding: 'utf8' });
