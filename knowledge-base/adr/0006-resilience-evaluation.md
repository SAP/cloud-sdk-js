...
Fallback           | **Yes**   | *Yes, but w/o error propagation* | *Yes, but w/o error propagation* | **Yes**
HystrixSSE Stream  | *Yes, but only if you use RXJS <6.0.0 otherwise runtime error due to changed Observable API* | **Yes** | No | **Yes**
Circuit Breaker    | **Yes**   | *Yes, but implementation is very odd ... After circuit breaker timeout everything is set to zero (?!?), e.g., there is no half open state of the breaker* | *Yes, but only two options to configure available* | **Yes**
Last Update | 2 years ago, still depends on RXJS < 6 | *Nov 9, 2018* | **Jan 13, 2019** | **Jan 31, 2019**
