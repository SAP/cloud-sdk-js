...
`GetAllRequestBuilder` -> `GetAllRequestBuilder`(`@sap-cloud-sdk/odata-v2`)
...
* We can already rename `GetAllRequestBuilder` -> `GetAllRequestBuilder`, etc. by exporting files like this `export {GetAllRequestBuilder as GetAllRequestBuilder}` (#before)
