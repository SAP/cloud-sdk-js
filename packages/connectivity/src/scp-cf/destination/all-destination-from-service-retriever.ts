...
import { DestinationFromServiceRetriever, SubscriberToken } from './destination-from-service';
import { fetchInstanceDestinations, fetchSubaccountDestinations } from './destination-service';
...
export async function getAllDestinationsFromDestinationService(
    ...
    const destinationSearchResult = await searchDestination(options, subscriberToken, providerToken);
...
async function searchDestination(
    ...
    if (isSubscriberNeeded(subscriberToken)) {
        logger.debug('Retrieving all subscriber destinations from destination service.');
        destinationSearchResult =
            await getInstanceAndSubaccountDestinations(destinationServiceUri, options, subscriberToken!.serviceJwt!.encoded);
    } else {
        logger.debug('Retrieving all provider destinations from destination service.');
        await getInstanceAndSubaccountDestinations(destinationServiceUri, options, providerToken!.encoded);
    }
