import { getSubdomainAndZoneId } from './xsuaa-service-xssec';
import * as jwt from './jwt';

describe('xsuaa-service-xssec', () => {
  beforeEach(() => {
    jest.resetModules();
  });
  ...
  it('returns subdomain and zoneId', () => {
    ...
    jest
      .spyOn(jwt, 'decodeJwt')
      .mockImplementationOnce(token => ({ iss, zid }));
  ...
  it('returns null zoneId, when the jwt does not contain zid', () => {
    const iss = 'https://sub.example.com';
    jest.spyOn(jwt, 'decodeJwt').mockImplementationOnce(token => ({ iss }));
  ...
  it('returns null subdomain, when the jwt does not contain iss', () => {
    const zid = 'zone';
    jest.spyOn(jwt, 'decodeJwt').mockImplementationOnce(token => ({ zid }));
