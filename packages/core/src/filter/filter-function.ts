...
import { Field, FieldODataV4, FieldType } from '../selectable';
...
export abstract class FilterFunctionODataV4<
  ...
  constructor(
    public functionName: string,
    public parameters: FilterFunctionParameterTypeODataV4<EntityT>[]
  ) {}
   ...
   * @returns The resulting filter
   */
  equals(value: ReturnT): FilterODataV4<EntityT, ReturnT> {
    return new FilterODataV4(this, 'eq', value, this.edmType);
  }
   ...
   * @returns The resulting filter
   */
  notEquals(value: ReturnT): FilterODataV4<EntityT, ReturnT> {
    return new FilterODataV4(this, 'ne', value, this.edmType);
  }
   ...
   */
  private transformParameter(
    param: FilterFunctionParameterTypeODataV4<EntityT>,
    parentFieldNames: string[]
  ): string {
    if (typeof param === 'number') {
      return param.toString();
      ...
      return convertToUriForEdmString(param);
    } else if (param instanceof FilterFunctionODataV4) {
      return param.toString(parentFieldNames);
    }
    return [...parentFieldNames, param._fieldName].join('/');
  }
...
export type FilterFunctionParameterTypeODataV4<EntityT extends EntityODataV4> =
  | number
  | string
  | FieldODataV4<EntityT>
  | FilterFunctionODataV4<EntityT, FieldType>;
