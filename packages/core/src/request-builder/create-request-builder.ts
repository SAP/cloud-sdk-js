 ...
 */
export class CreateRequestBuilder<EntityT extends Entity> extends MethodRequestBuilderBase<ODataCreateRequestConfig<EntityT>>
   ...
   */
  constructor(readonly _entityConstructor: Constructable<EntityT>, readonly entity: EntityT) {
   ...
   */
  prepare(): this {
    this.requestConfig.payload = serializeEntity(this.entity, this._entityConstructor);
    return this;
  }
  ...
  asChildOf<ParentEntityT extends Entity>(parentEntity: ParentEntityT, linkField: Link<ParentEntityT, EntityT>): this {
    this.requestConfig.parentKeys = getEntityKeys(parentEntity, linkField._entityConstructor);
  ...
  async execute(destination: Destination | DestinationNameAndJwt, options?: DestinationOptions): Promise<EntityT> {
    ...
    return this.build(destination, options)
      ...
      .then(response => deserializeEntity(response.data.d, this._entityConstructor, response.headers))
      .catch(error => Promise.reject(errorWithCause('Create request failed!', error)));
  }
}
