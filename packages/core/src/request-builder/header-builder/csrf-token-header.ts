...
const csrfTokenAndCookies = <RequestT extends ODataRequestConfig>(
  request: ODataRequest<RequestT>
) => (headers: MapType<string>) =>
  asyncPipe(
    getCsrfTokenResponseHeaders(request),
    validateCsrfTokenResponse,
    mergeTokenAndCookies(headers)
  )(headers);

const getCsrfTokenResponseHeaders = <RequestT extends ODataRequestConfig>(
  request: ODataRequest<RequestT>
) => (authHeader: MapType<string>): Promise<MapType<any>> => {
...
const addCsrfFetchHeader = (headers: MapType<string>) =>
  assoc('x-csrf-token', 'Fetch')(headers);

const retrieveCsrfTokenHeaders = (
...
const mergeTokenAndCookies = (headers: MapType<string>) => (
  csrfResponseHeaders: MapType<any>
) =>
  pipe(
    addCsrfTokenHeader(csrfResponseHeaders['x-csrf-token']),
    addCookieHeader(csrfResponseHeaders['set-cookie'])
  )(headers);

const addCsrfTokenHeader = (csrfToken?: string) => (headers: MapType<string>) =>
  assocSome('x-csrf-token', csrfToken)(headers);
const addCookieHeader = (cookies?: string[]) => (headers: MapType<string>) =>
  assocSome('cookie', buildCookieHeaderValue(cookies))(headers);

const buildCookieHeaderValue = (cookies?: string[]): string | undefined =>
  cookies && cookies.length
    ? cookies.map((cookie: string) => head(cookie.split(';'))).join(';')
    : undefined;
