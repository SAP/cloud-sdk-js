...
import { addAuthorizationHeader, buildAndAddAuthorizationHeader } from './authorization-header';
...
export function addSapClientHeader(sapClient?: string) {
  return (headers: MapType<string>) => assocSome('sap-client', sapClient)(headers);
}

export const addProxyHeaders = (proxyConfiguration?: ProxyConfiguration) => (headers: MapType<string>) =>
  mergeSome(headers, path(['headers'], proxyConfiguration));

export const addLocationIdHeader = (locationId?: string) => (headers: MapType<string>) =>
  assocSome('SAP-Connectivity-SCC-Location_ID', locationId)(headers);

const buildHeadersAuthFirstCustomLast = <RequestT extends ODataRequestConfig>(request: ODataRequest<RequestT>) => (
  ...fns
): Promise<MapType<string>> =>
  asyncPipe(headers => addAuthorizationHeader(request, headers), ...fns, addCustomHeaders(request.config.customHeaders))({});

const addEtagHeader = (requestConfig: ODataRequestConfig) => (headers: MapType<string>) =>
  assocSome('if-match', getETagHeader(requestConfig))(headers);

const addCustomHeaders = (customHeaders: MapType<string>) => (headers: MapType<string>) => ({
  ...headers,
  ...customHeaders
});

const addContentTypeHeader = (contentType: string, headers: MapType<string>) => assoc('Content-Type', contentType)(headers);

const addAcceptHeader = (headers: MapType<string>) => assoc('Accept', 'application/json')(headers);

const eTagFromConfig = <T extends Entity>(config: ODataUpdateRequestConfig<T> | ODataDeleteRequestConfig<T>) =>
  config.versionIdentifierIgnored ? '*' : config.eTag;

const getETagHeader = (config: ODataRequestConfig): string | undefined =>
  ifElse(
    c => c instanceof ODataUpdateRequestConfig || c instanceof ODataDeleteRequestConfig,
    eTagFromConfig,
    () => undefined
  )(config);
