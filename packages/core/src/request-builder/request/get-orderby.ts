...
function getODataOrderByExpressions<OrderByEntityT extends Entity>(
  ...
  return orderBys.reduce((expressions: string[], orderBy: Orderable<OrderByEntityT>) => {
    ...
    return [...expressions, getOrderByExpressionForOrder(orderBy, parentFieldNames)];
  }, []);
}

function getOrderByExpressionForOrderLink<OrderByEntityT extends Entity, LinkedEntityT extends Entity>(
  orderBy: OrderLink<OrderByEntityT, LinkedEntityT>,
  parentFieldNames: string[] = []
): string {
  return getODataOrderByExpressions(orderBy.orderBy, [...parentFieldNames, orderBy.link._fieldName]).join(',');
}

function getOrderByExpressionForOrder<OrderByEntityT extends Entity>(orderBy: Order<OrderByEntityT>, parentFieldNames: string[] = []): string {
  return [[...parentFieldNames, orderBy._fieldName].join('/'), orderBy.orderType].join(' ');
}
