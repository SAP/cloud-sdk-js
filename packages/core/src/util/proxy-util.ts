...
const addProtocol = (groups: any) => (proxyConfiguration: Partial<ProxyConfiguration> | undefined) => {
...
const addPort = (groups: any) => (proxyConfiguration: Partial<ProxyConfiguration> | undefined) => {
  if (!proxyConfiguration) {
    return;
  }
  ...
  const fallBackPort = proxyConfiguration.protocol === Protocol.HTTPS ? 443 : 80;
  copy.port = fallBackPort;
  logger.info(`Port not specified in proxy environment value. ${fallBackPort} used as fallback.`);
  return copy;
};

const addAuthHeaders = (groups: any) => (proxyConfiguration: Partial<ProxyConfiguration> | undefined) => {
  if (!proxyConfiguration) {
    return;
  }
  const copy = { ...proxyConfiguration };
  ...
  copy.headers = { 'Proxy-Authorization': basicHeader(userDecoded, pwdDecoded) };
  logger.info('Username and password added to authorization of the proxy configuration.');
  return copy;
};

const addHost = (groups: any) => (proxyConfiguration: Partial<ProxyConfiguration>) => {
  if (groups.host) {
...
export function parseProxyEnv(proxyEnvValue: string): ProxyConfiguration | undefined {
  ...
  if (parsed?.groups) {
    ...
    const proxyConfiguration = pipe(addHost(groups), addProtocol(groups), addPort(groups), addAuthHeaders(groups))({});
