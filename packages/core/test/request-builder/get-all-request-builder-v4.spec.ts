...
import { createPerson, createPersonJson1, createPersonJson2 } from '../test-util/test-data-trip-service';
...
describe('GetAllRequestBuilder', () => {
  ...
  describe('url', () => {
    it('is built correctly', async () => {
      const expected =
        '/testination/TripPinRESTierService/Person?$format=json';
  ...
  describe('execute', () => {
    it('returns all entities', async () => {
      ...
      const actual = await new GetAllRequestBuilder(Person).execute(defaultDestination);
