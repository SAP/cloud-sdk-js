...
import { TestEntity, TestEntityLvl2MultiLink, TestEntityMultiLink, TestEntitySingleLink } from '../../test-util/test-services/test-service';
...
describe('get selection', () => {
  ...
  it('for simple selects', () => {
    expect(getQueryParametersForSelection([TestEntity.STRING_PROPERTY, TestEntity.BOOLEAN_PROPERTY]).select).toBe('StringProperty,BooleanProperty');
  });

  it('removes reduncancy for selection of all fields', () => {
    expect(getQueryParametersForSelection([TestEntity.ALL_FIELDS, TestEntity.BOOLEAN_PROPERTY]).select).toBe('*');
  });
  ...
  it('for multilevel selects', () => {
    ...
    expect(params.select).toBe('StringProperty,BooleanProperty,to_SingleLink/StringProperty');
  ...
  it('for nested multilevel selects', () => {
    const params = getQueryParametersForSelection([
      TestEntity.STRING_PROPERTY,
      TestEntity.BOOLEAN_PROPERTY,
      TestEntity.TO_MULTI_LINK.select(TestEntityMultiLink.TO_MULTI_LINK.select(TestEntityLvl2MultiLink.INT_16_PROPERTY))
    ]);
    expect(params.select).toBe('StringProperty,BooleanProperty,to_MultiLink/to_MultiLink/Int16Property');
  ...
  it('multilevel selects are not removed when there is a multilevel all field selection', () => {
    const params = getQueryParametersForSelection([
      TestEntity.BOOLEAN_PROPERTY,
      TestEntity.TO_SINGLE_LINK.select(TestEntitySingleLink.ALL_FIELDS, TestEntitySingleLink.INT_16_PROPERTY)
    ]);
  ...
  it('for selects link entity', () => {
    const params = getQueryParametersForSelection([TestEntity.STRING_PROPERTY, TestEntity.TO_SINGLE_LINK]);
    expect(params.select).toBe('to_SingleLink/*,StringProperty');
    expect(params.expand).toBe('to_SingleLink');
  });

  it('for select all field link entity ', () => {
    const params = getQueryParametersForSelection([TestEntity.STRING_PROPERTY, TestEntity.TO_SINGLE_LINK.select(TestEntitySingleLink.ALL_FIELDS)]);
    expect(params.select).toBe('to_SingleLink/*,StringProperty');
    expect(params.expand).toBe('to_SingleLink');
  });

  it('for custom field selects', () => {
    expect(getQueryParametersForSelection([TestEntity.customField('SomeCustomField'), TestEntity.BOOLEAN_PROPERTY]).select).toBe(
      'SomeCustomField,BooleanProperty'
    );
  });
