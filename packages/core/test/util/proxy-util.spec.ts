...
import { addProxyConfigurationInternet, parseProxyEnv, ProxyStrategy, proxyStrategy } from '../../src/util/proxy-util';
...
describe('proxy-util', () => {
  ...
  it('should use proxy type OnPrem if the destination is onPrem - even if HTTP(S)_PROXY env are present.', () => {
    ...
    expect(proxyStrategy(onPremDestination)).toBe(ProxyStrategy.ON_PREMISE_PROXY);
    expect(addProxyConfigurationInternet(onPremDestination)).toStrictEqual({
      ...onPremDestination,
      proxyConfiguration: { protocol: Protocol.HTTPS, host: 'some.proxy.com', port: 443 }
    });
  });
  ...
  it('should use no proxy if the destination URL is in the NO_PROXY list.', () => {
    ...
    expect(addProxyConfigurationInternet(httpsDestination)).toStrictEqual({
      ...httpsDestination,
      proxyConfiguration: { protocol: Protocol.HTTPS, host: 'some.proxy.com', port: 4711 }
    });

    process.env['no_proxy'] = `http://some.otherURL.com,${httpsDestination.url}`;
  ...
  it('should use the proxy env with the  protocol indicated by the destination.', () => {
    ...
    expect(addProxyConfigurationInternet(httpsDestination)).toStrictEqual({
      ...httpsDestination,
      proxyConfiguration: { protocol: Protocol.HTTP, host: 'some.https.proxy', port: 1234 }
    });
    expect(addProxyConfigurationInternet(httpDestination)).toStrictEqual({
      ...httpDestination,
      proxyConfiguration: { protocol: Protocol.HTTP, host: 'some.http.com', port: 80 }
    });
  });
  ...
  it('should include user/pwd from proxy to configuration.', () => {
    ...
    expect(addProxyConfigurationInternet(httpsDestination)).toStrictEqual({
      ...httpsDestination,
      proxyConfiguration: { protocol: Protocol.HTTPS, host: 'some.proxy.com', port: 443, headers: { 'Proxy-Authorization': userPwdEncoded } }
    });

    process.env['https_proxy'] = 'user:password@some.proxy.com';
    expect(addProxyConfigurationInternet(httpsDestination)).toStrictEqual({
      ...httpsDestination,
      proxyConfiguration: { protocol: Protocol.HTTP, host: 'some.proxy.com', port: 80, headers: { 'Proxy-Authorization': userPwdEncoded } }
    });
  });
  ...
  it('should allow for special characters in the user and password if they are percent encoded.', () => {
    ...
    expect(addProxyConfigurationInternet(httpsDestination).proxyConfiguration).toBe(undefined);

    process.env['https_proxy'] = 'https://us%40er:pass%3A%2Fword@some.proxy.com:443';
    expect(addProxyConfigurationInternet(httpsDestination)).toStrictEqual({
      ...httpsDestination,
      proxyConfiguration: { protocol: Protocol.HTTPS, host: 'some.proxy.com', port: 443, headers: { 'Proxy-Authorization': userPwdEncoded } }
    });
  ...
  describe('regex testing', () => {
    it('should parse the protocol with defaults', () => {
      ...
      expect(parseProxyEnv('https://some.https.proxy.with.port:1234')?.protocol).toBe('https');
      expect(parseProxyEnv('http://some.http.proxy')?.protocol).toBe('http');

      expect(parseProxyEnv('http://user:pwd@some.http.proxy.with.password:4711')?.protocol).toBe('http');
      expect(parseProxyEnv('rfc://unsupported.protocol.No.Proxy')).toBe(undefined);
    });
    ...
    it('should parse  port with default.', () => {
      ...
      expect(parseProxyEnv('http://user:pwd@some.http.proxy.with.password')?.port).toBe(80);
      expect(parseProxyEnv('https://user:pwd@some.http.proxy.with.password')?.port).toBe(443);
      ...
      expect(parseProxyEnv('http://user:pwd@some.http.proxy.with.password:1234')?.port).toBe(1234);
    ...
    it('should parse host if environment value is ok.', () => {
      ...
      expect(parseProxyEnv('http://user:password@some.host:1234')?.host).toBe('some.host');
      expect(parseProxyEnv('user:password@some.host:1234')?.host).toBe('some.host');
    ...
    it('should parse user and password', () => {
      const userPwdEncoded = basicHeader('user', 'password');
      expect(parseProxyEnv('user:password@some.proxy.com')!.headers!['Proxy-Authorization']).toBe(userPwdEncoded);
      expect(parseProxyEnv('https://user:password@some.proxy.com')!.headers!['Proxy-Authorization']).toBe(userPwdEncoded);
      expect(parseProxyEnv('user:password@some.proxy.com:443')!.headers!['Proxy-Authorization']).toBe(userPwdEncoded);
      expect(parseProxyEnv('https://user:password@some.proxy.com:443')!.headers!['Proxy-Authorization']).toBe(userPwdEncoded);

      expect(parseProxyEnv('password@some.proxy.com')).toBe(undefined);
      expect(parseProxyEnv('u@ser:password@some.proxy.com:443')).toBe(undefined);
    });
  });
});
