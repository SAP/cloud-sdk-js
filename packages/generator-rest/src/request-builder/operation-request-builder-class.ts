import { OpenApiOperation, OpenApiPath, OpenApiServiceMetadata } from '../open-api-types';
import {
...
import { pathParamToParamStructure, refNameToParamStructure } from './api-request-builder-class';
  ...
  return {
    ...
    ctors: [{
      parameters:[
        ...
        ...addPublicScope(pathParamToParamStructure(openApiPath.pathParameters)),
        ...addPublicScope(refNameToParamStructure(operation.requestBodySchemaRefName))
      ],
...
function addPublicScope(objs: OptionalKind<ParameterDeclarationStructure>[]): OptionalKind<ParameterDeclarationStructure>[]{
  return objs.map(o => {
    return {...o, scope: Scope.Public}
  })
}

function method(metadata: OpenApiServiceMetadata, openApiPath: OpenApiPath, operation: OpenApiOperation): MethodDeclarationStructure[]{
  return [{
...
function toStatement(metadata: OpenApiServiceMetadata, openApiPath: OpenApiPath, operation: OpenApiOperation){
  const parameters = [
    ...buildParameterFromPathParameters(openApiPath.pathParameters),
    ...buildParameterFromRefName(operation.requestBodySchemaRefName),
    'requestConfig'
  ];
  return `const requestConfig: AxiosRequestConfig = await this.buildRequestConfig(destination);\n` +
...
function getRequestConfigParameter(): OptionalKind<ParameterDeclarationStructure>{
    return {
      name: 'requestConfig',
