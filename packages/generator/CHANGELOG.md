...
- 94b45b10b: [Compatibility Note] The options per service behavior is now the same as for the OpenApi generator.
  For details on how to migrate and what has change look at the migration guide.
- 94b45b10b: [Compatibility Note] The deprecated `forceOverwrite` option of the generator is removed. Use the `overwrite` option instead.
...
- 94b45b10b: [Compatibility Note] Add `verbose` generator option. By default, only error and warning logs will be displayed. If `verbose` is set to `true`, all logs will be displayed.
- 94b45b10b: [New Functionality] Introduced options `transpile` and `tsconfig` to configure transpilation for generated OData clients.
- 94b45b10b: [Compatibility Note] The `generateJs` option has been replaced with the `transpile` option. Transpilation is not enabled by default.
- 94b45b10b: [Compatibility Note] The command line argument `inputDir` of the OData generator is renamed to `input`.
  The new `input` options accepts now also file paths and glob patterns.
- 94b45b10b: [Compatibility Note] The OData generator won't automatically rename identifiers to avoid name conflicts.
...
- 94b45b10b: [Compatibility Note] Removed the option `versionInPackageJson` from the OData and OpenAPI generator.
  ...
  If necessary use the `include` option to add a `package.json` with a custom value.
- 94b45b10b: [Compatibility Note] Rename servicePath to basePath.
...
- 94b45b10b: [Compatibility Note] The option `generateCSN` is removed. There is no replacement.
- 94b45b10b: [Compatibility Note] The `serviceMapping` option of the OData generator has been renamed to `optionsPerService`. The mapping file, `service-mapping.json` has also been renamed to `options-per-service.json`. By default, an options file will not be generated.
- 94b45b10b: [Compatibility Note] The type for paths in the `GeneratorOptions` is changed from `fs.PathLike` to `string`.
  In case you passed a buffer object please resolve it to a string before passing it to the SAP Cloud SDK.
- 94b45b10b: [Compatibility Note] The two generators use the same CLI parsing code now, aligning the way paths are resolved.
  In case you experience problems with the new logic, enable the `verbose` flag to investigate the new paths.

### Minor Changes

- 94b45b10b: [Improvement] Align naming rules for operations and properties in OData clients by removing `_` prefix
- 94b45b10b: [Fixed Issue] Allow OData service to contain an entity name 'Service'.

### Patch Changes

- 94b45b10b: [Fixed Issue] Now links to the correct generator binary.
