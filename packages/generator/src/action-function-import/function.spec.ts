...
describe('function', () => {
  it('functionImportFunction', () => {
    ...
    expect(actual).toEqual({
      ...
      isExported: true,
      statements:
        `const params = {${EOL}withHoneyToast: new FunctionImportParameter('WithHoneyToast', 'Edm.Boolean', parameters.withHoneyToast)${EOL}}${EOL}${EOL}return new FunctionImportRequestBuilderV2('post', 'some/path/to/food', 'OrderBreakfast', (data) => transformReturnValueForEdmTypeV2(data, (val) => edmToTs(val, Edm.String)), params);`
    });
  });
});
