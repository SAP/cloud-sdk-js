...
export interface FindUniqueName {
...
function castToUniqueName(findUniqueName: FindUniqueName): UniqueName {
  ...
  return {
    uniqueName: name,
    relatedUniqueNames: findUniqueName.relatedNamesBuilder
      ? findUniqueName.relatedNamesBuilder(name)
      : []
  };
}
...
function isNameAndRelatedUnique(findUniqueName: FindUniqueName): boolean {
  ...
  return !allNames.some(name =>
    isNameAlreadyUsed(name, findUniqueName.alreadyUsedNames)
  );
}
...
function addSuffix(
  findName: FindUniqueName,
  suffix: number,
  separator: Separator
): FindUniqueName {
  const nameWithoutSuffix = removeSuffixIfPresent(
    findName.nameToCheckForUniqueness,
    separator
  );
  return {
    ...findName,
...
function removeUnnecessaryUsedNames(
  findUniqueName: FindUniqueName,
  separator: Separator
): FindUniqueName {
  const nameNoSuffix = removeSuffixIfPresent(
    findUniqueName.nameToCheckForUniqueness,
    separator
  );
  const relevantAlreadyUsedItems = findUniqueName.alreadyUsedNames.filter(
    name => name.startsWith(nameNoSuffix)
  );
  return {
    ...findUniqueName,
...
function prepareDataForSuffixSearch(
  findUniqueName: FindUniqueName,
  separator: Separator
): FindUniqueName {
  const nameNoSuffix = removeSuffixIfPresent(
...
function getUniqueNameUsingSuffix(
  findUniqueName: FindUniqueName,
  separator: Separator
): UniqueName {
  let suffix = 1;
  ...
  throw new Error(
    `Unable to find a unique name for ${findUniqueName.nameToCheckForUniqueness} within the range of 1000 suffixes.`
  );
}
