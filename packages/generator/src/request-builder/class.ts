...
export function requestBuilderClass(entity: VdmEntity): ClassDeclarationStructure {
  return {
...
function requestBuilderMethods(entity: VdmEntity): MethodDeclarationStructure[] {
  const methods = [getByKeyRequestBuilder(entity), getAllRequestBuilder(entity)];
  if (entity.creatable) {
...
function getByKeyRequestBuilder(entity: VdmEntity): MethodDeclarationStructure {
  return {
    ...
    docs: [
      getFunctionDoc(`Returns a request builder for retrieving one \`${entity.className}\` entity based on its keys.`, {
        returns: {
...
function getAllRequestBuilder(entity: VdmEntity): MethodDeclarationStructure {
  return {
    ...
    docs: [
      getFunctionDoc(`Returns a request builder for querying all \`${entity.className}\` entities.`, {
        returns: {
...
function createRequestBuilder(entity: VdmEntity): MethodDeclarationStructure {
  return {
    ...
    docs: [
      getFunctionDoc(`Returns a request builder for creating a \`${entity.className}\` entity.`, {
        returns: {
...
function updateRequestBuilder(entity: VdmEntity): MethodDeclarationStructure {
  return {
    ...
    docs: [
      getFunctionDoc(`Returns a request builder for updating an entity of type \`${entity.className}\`.`, {
        returns: {
...
function deleteRequestBuilderOverload(entity: VdmEntity): MethodDeclarationOverloadStructure[] {
  return [
    {
      ...
      docs: [
        getFunctionDoc(`Returns a request builder for deleting an entity of type \`${entity.className}\`.`, {
          returns: {
...
function deleteRequestBuilderStatements(entity: VdmEntity) {
  ...
  const keys = entity.keys.map((key, index) => `${key.originalName}: ${params[index]}!`).join(',\n');
  return `return new DeleteRequestBuilder(${entity.className}, ${params[0]} instanceof ${entity.className} ? ${params[0]} : {${keys}});`;
}

function buildParametrizedStatements(entity: VdmEntity, requestBuilder: RequestBuilderName) {
  const params = entity.keys.map(key => `${key.originalName}: ${key.propertyNameAsParam}`).join(',\n');
