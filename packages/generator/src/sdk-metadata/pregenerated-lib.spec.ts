...
import {
  ...
  getTimeStamp, isPublishedNpmPackage, getPregeneratedLibrary
...
describe('pregenerated-lib',()=>{
  ...
  it('returns version from generator options or generator version',()=>{
    expect(getVersionForClient({ versionInPackageJson:'123' } as GeneratorOptions)).toBe('123');
    expect(getVersionForClient({} as GeneratorOptions)).toMatch(/\d+\.\d+\.\d+/);
  });
  ...
  it('[E2E] return pregenerated lib information',async ()=>{
    const result = await getPregeneratedLibrary(getTestService('@sap/cloud-sdk-vdm-business-partner-service'),createOptions());
