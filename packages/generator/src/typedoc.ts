...
import { VdmComplexType, VdmEntity, VdmNavigationProperty, VdmProperty, VdmPropertyValueConstraints, VdmServiceMetadata } from './vdm-types';

export function getFunctionDoc(description: string, tags: Partial<{ typeparams: DocType[]; params: NamedDocType[]; returns: DocType }>): string {
  if (tags) {
    if (tags.typeparams) {
...
export function getPropertyDescription(property: VdmProperty, constraints: VdmPropertyValueConstraints = { nullable: false }): string {
  return addConstraints(property.description || endWithDot(toTitleFormat(property.instancePropertyName).trim()), constraints);
}

export function getNavPropertyDescription(property: VdmNavigationProperty): string {
  return `${property.isMultiLink ? 'One-to-many' : 'One-to-one'} navigation property to the [[${property.toEntityClassName}]] entity.`.trim();
}

export function getComplexTypePropertyDescription(property: VdmProperty, complexTypeName: string): string {
...
export function getStaticNavPropertyDescription(property: VdmNavigationProperty): string {
  return `Static representation of the ${property.isMultiLink ? 'one-to-many' : 'one-to-one'} navigation property [[${
    property.instancePropertyName
  }]] for query construction.\nUse to reference this property in query operations such as 'select' in the fluent request API.`;
}

export function getEntityDescription(entity: VdmEntity, service: VdmServiceMetadata): string {
  let description = entityDescription(entity.entitySetName, service.namespace);

  if (service.apiBusinessHubMetadata && service.apiBusinessHubMetadata.communicationScenario) {
    description += '\n' + partOfCommunicationScenarios(service.apiBusinessHubMetadata.communicationScenario);
  }
...
const seeForMoreInformation = (url: string) => `See ${url} for more information.`;
...
function addConstraints(description: string, constraints: VdmPropertyValueConstraints): string {
