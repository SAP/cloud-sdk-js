...
describe('entity class generator', () => {
  it('creates a class', () => {
    ...
    const staticProperties = classDeclaration.properties!.filter(prop => prop.isStatic);
    ...
    const instanceProperties = classDeclaration.properties!.filter(prop => !prop.isStatic);
    ...
    expect(classDeclaration.methods!.map(method => method.name)).toEqual(['builder', 'requestBuilder', 'customField', 'toJSON']);
  });
});
