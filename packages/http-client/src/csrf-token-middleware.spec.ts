...
describe('CSRF middleware',()=>{
    ...
    it('does nothing for GET requests',async ()=>{
        ...
        await executeHttpRequest({ url: host },{ method:'GET',url:'some/path' });
    ...
    it('does nothing if header is already present',async ()=>{
        ...
        const spy = jest.spyOn(axios, 'request');
        await executeHttpRequest({ url: host },{ method:'POST',url:'some/path',headers:{ 'x-csrf-token':'tokenAlreadyThere' } });
        expect(spy).toBeCalledTimes(1);
    });
    ...
    it('does not fail if header can not fetched from response',async ()=>{
        ...
        nock(host)
            .post('/some/path')
            .reply(200,  {});
        await expect(executeHttpRequest({ url: host },{ method:'POST',url:'some/path' })).resolves.not.toThrow();
    });
    ...
    it('fetches and adds the csrf token without cookie',async ()=>{
        ...
        const spy = jest.spyOn(axios, 'request');
        await executeHttpRequest({ url: host },{ method:'POST',url:'some/path' });
        expect(spy).toHaveBeenNthCalledWith(2,expect.objectContaining({ method:'POST',headers:csrfResponseHeaders, url: 'some/path' }));
    });
    ...
    it('fetches and adds the csrf token with cookie',async ()=>{
        ...
        const spy = jest.spyOn(axios, 'request');
        await executeHttpRequest({ url: host },{ method:'POST',url:'some/path' });
        expect(spy).toHaveBeenNthCalledWith(2,expect.objectContaining({ method:'POST',headers:{ ...csrfResponseHeaders,cookie:'cookieValue' }, url: 'some/path' }));
    });
    ...
    it('fetches headers from "/some/path/" first',async ()=>{
        ...
        const spy = jest.spyOn(axios, 'request');
        await executeHttpRequest({ url: host },{ method:'POST',url:'some/path' });
        expect(spy).toHaveBeenNthCalledWith(1,expect.objectContaining({ method:'head',headers:csrfFetchHeader, url: 'some/path/' }));
    });
    ...
    it('fetches header from "/some/path" if "/some/path/" fails',async ()=>{
        ...
        const spy = jest.spyOn(axios, 'request');
        await executeHttpRequest({ url: host },{ method:'POST',url:'some/path' });
        expect(spy).toHaveBeenNthCalledWith(2,expect.objectContaining({ method:'head',headers:csrfFetchHeader, url: 'some/path' }));
    });
    ...
    it('fetches headers without parameters at resource URL',async ()=>{
        ...
        nock(host)
            .post('/some/path?paraKey=paraValue')
            .reply(200,  {}
            );
        const spy = jest.spyOn(axios, 'request');
        await executeHttpRequest({ url: host },{ method:'POST',url:'some/path',params:{ paraKey:'paraValue' } });
        expect(spy).toHaveBeenNthCalledWith(1,expect.objectContaining({ method:'head',headers:csrfFetchHeader, url: 'some/path/' }));
    });

    it('fetches the token from custom URL',async()=>{
        nock(host)
            .head('/alternative/path')
            .reply(200,  {},csrfResponseHeaders
            );
        nock(host)
            .post('/some/path')
        ...
        await expect(executeHttpRequest({ url: host },{ method:'POST',url:'some/path',middleware:[csrf({ url:'http://example.com/alternative/path' })] },{ fetchCsrfToken:false })).resolves.not.toThrow();
    });
    ...
    it('fetches the token with custom method',async()=>{
        ...
        await expect(executeHttpRequest({ url: host },{ method:'POST',url:'some/path',middleware:[csrf({ method:'GET' })] },{ fetchCsrfToken:false })).resolves.not.toThrow();
    });
});
