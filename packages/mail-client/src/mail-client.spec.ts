...
describe('mail client', () => {
  ...
  it('should create transport, send mails and close the transport', async () => {
    const spyCreateTransport = jest.spyOn(nodemailer, 'createTransport').mockReturnValue(new MockMailer() as any);
    ...
    const mailOptions1: MailOptions = {from: 'from1@example.com', to: 'to1@example.com'}
    const mailOptions2: MailOptions = {from: 'from2@example.com', to: 'to2@example.com'}
    await expect(sendMail(destination, mailOptions1, mailOptions2)).resolves.not.toThrow();
