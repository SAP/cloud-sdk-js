...
- 1b974095f: [Compatibility Note] The constructor of `ODataRequestConfig` was changed so that the third parameter cannot be a `string` anymore.
...
- 1b974095f: [Compatibility Note] The constructor of the entity API is private to avoid accidental usage. Use the service object to get a API instance.
...
- 1b974095f: [Compatibility Note] Remove `timeout()` method from the request builders and the `timeout` options from the `executeHttpRequest()` function.
...
- 1b974095f: [Compatibility Note] Setting custom fields in `fromJson` through the `_customFields` property has been removed.
...
- 1b974095f: [New Functionality] Introduce the middleware on the request builders and http-client.
