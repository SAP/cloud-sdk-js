...
export function createValueSerializer<DeSerializersT extends DeSerializers>(
  deSerializers: DeSerializersT
): <EdmT>(value: any, edmType: EdmT, propertyMetadata?: any) => any {
  return (value, edmType, propertyMetadata) => {
    ...
    if (edmType === 'Edm.DateTimeOffset' && propertyMetadata) {
      return serialize(value, propertyMetadata);
    }
