...
describe('batch request',()=>{
    ...
    const testEntity = testEntityApi.entityBuilder().stringProperty('4711').build();
...
`;
    ...
    it('executes a getAll request',async()=>{
        nock(baseUrl).post('/sap/opu/odata/sap/API_TEST_SRV/$batch').reply(202,getAllResponse,{ 'content-type':`multipart/mixed; boundary=${boundary}` });
        const response = await batch(testEntityApi.requestBuilder().getAll()).execute({ url:baseUrl });
