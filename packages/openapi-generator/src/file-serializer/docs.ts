...
import { OpenApiApi, OpenApiDocument, OpenApiOperation, OpenApiParameter, OpenApiRequestBody } from '../openapi-types';
...
function getSignatureOfPathParameters(parameters: OpenApiParameter[]): string[]{
  return parameters.map((parameter,i)=>`@param ${parameter.name} ${parameter.description || `Path parameter number ${i+1}`}`);
}

function getSignatureOfBody(body: OpenApiRequestBody): string{
  return `@param body ${body.required ? 'Object' : 'Optional object'} containing the request body of type '${serializeSchema(body.schema)}'`;
}
