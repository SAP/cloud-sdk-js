...
describe('index-file', () => {
  ...
  it('apiIndexFile serializes the api index file following the esm format', async () => {
    const document = {
      apis: [{ name: 'TestApi' }, { name: 'DefaultApi' }],
      schemas: [{}]
    } as OpenApiDocument;
  ...
  it('schemaIndexFile serializes the schema index file for schemas following the ESM format', async () => {
    const document = {
      apis: [],
      schemas: [
        { schemaName: 'MySchema1', fileName: 'my-schema-1' },
