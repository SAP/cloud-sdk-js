  ...
  it('uses circuit breaker and timeout by default with resilience()', async () => {
    ...
    while (keepCalling) {
      await expect(
        ...
        >(resilience({ timeout: delay * 0.5 }), context, request)
      ).rejects.toThrow();
    ...
    await expect(
      executeWithMiddleware<
        ...
        AxiosResponse,
        MiddlewareContext<RawAxiosRequestConfig>
      >(resilience({ timeout: false }), context, request)
  ...
  it('retry and circuit breaker are not used for 4xx error', async () => {
    ...
    await expect(
      executeWithMiddleware<
        ...
        AxiosResponse,
        MiddlewareContext<RawAxiosRequestConfig>
      >(resilience({ retry: true }), context, request)
  ...
  it('does 3 retries and does not open the circuit breaker for few server errors', async () => {
    ...
    await expect(
      executeWithMiddleware<
        ...
        AxiosResponse,
        MiddlewareContext<RawAxiosRequestConfig>
      >(resilience({ timeout: false, retry: true }), context, request)
