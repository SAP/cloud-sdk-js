...
describe('check-public-api',()=>{
    it('finds all index.ts',async()=>{
        ...
        expect(indexFiles('root')).toEqual(['root/folder1/folder2/index.ts', 'root/index.ts']);
        mock.restore();
    });
    ...
    it('fails for too many index files',()=>{
        ...
        expect(()=>checkSingleIndexFile('root')).toThrowError('Too many index files found: root/folder1/index.ts,root/index.ts');
        mock.restore();
    });
    ...
    it('fails if index file is not in root',()=>{
        ...
        expect(()=>checkSingleIndexFile('root')).toThrowError('Index file is not in root foldes root/folder1/index.ts');
        mock.restore();
    });
    ...
    it('finds the .d.ts files and excludes index.d.ts',()=>{
        expect(typeDescriptorPaths(resolve(__dirname,'../dist')).length).toBeGreaterThan(0);
        expect(typeDescriptorPaths(resolve(__dirname,'../dist')).find(path=>path.includes('index.d.ts'))).toBeUndefined();
    });

    it('parses one .d.ts file', ()=>{
        const exportedType = parseTypeDefinitionFile(dummyTypeDefinition);
        expect(exportedType).toEqual(['IsolationStrategy', 'parseTypeDefinitionFile', 'CacheEntry']);
    });
